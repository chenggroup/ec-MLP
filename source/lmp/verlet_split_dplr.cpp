...
VerletSplitDplr::VerletSplitDplr(LAMMPS *lmp, int narg, char **arg) :
  Verlet(lmp, narg, arg), qsize(nullptr), qdisp(nullptr), xsize(nullptr), xdisp(nullptr), f_kspace(nullptr)
{
  ...
  memory->create(rspace_grid2proc,rspace_procgrid[0],
                 ...
                 "verlet/split/dplr:rspace_grid2proc");
  ...
  memory->create(kspace_sublo_list, ratio, 3, "verlet/split/dplr:kspace_sublo_list");
  memory->create(kspace_subhi_list, ratio, 3, "verlet/split/dplr:kspace_subhi_list");

  memory->create(sortbin_lo, 3, ratio, "verlet/split/dplr:sortbin_lo");
  memory->create(sortbin_hi, 3, ratio, "verlet/split/dplr:sortbin_hi");
...
void VerletSplitDplr::setup_minimal(int flag)
{
...
void VerletSplitDplr::run(int n)
{
  bigint ntimestep;
  int nflag,sortflag;
...
void VerletSplitDplr::neigh_comm(int nflag,int n_pre_exchange,int n_pre_neighbor)
{
      int sortflag;
      bigint ntimestep;
...
double VerletSplitDplr::memory_usage()
{
...
void VerletSplitDplr::setup_kspace_bins(
...
void VerletSplitDplr::resort_rspace_atom(double **sortbin_lo, double **sortbin_hi) {
  if(master){
  ...
  if (nlocal > maxnext) {
    ...
    memory->create(next, maxnext, "verlet/split/dplr:next");
    memory->create(permute, maxnext, "verlet/split/dplr:permute");
  }

  if (ratio > maxbin) {
    memory->destroy(binhead);
    maxbin = ratio;
    memory->create(binhead, maxbin, "verlet/split/dplr:binhead");
  }
